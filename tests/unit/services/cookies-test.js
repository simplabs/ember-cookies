...
describe('CookiesService', function() {
  ...
  function itReadsAfterWrite() {
    it('reads a cookie that was just written', function() {
      ...
      expect(this.cookies.read(COOKIE_NAME)).to.eq(value);
    });
  }

  describe('in the browser', function() {
    function itUsesBrowserAPIs() {
      beforeEach(function() {
        this.fakeDocument = {
      ...
      describe('reading a cookie', function() {
        ...
        it('handles invalid cookies', function() {
          ...
          expect(this.cookies.read('')).to.deep.equal({});
        });
        ...
        it('returns undefined for a cookie that was written for another path', function() {
          ...
          expect(this.cookies.read(COOKIE_NAME)).to.be.undefined;
        });
      ...
      describe("checking for a cookie's existence", function() {
        it('returns true when the cookie exists', function() {
          ...
          expect(this.cookies.exists(COOKIE_NAME)).to.be.true;
        });
        ...
        it('returns false when the cookie does not exist', function() {
          expect(this.cookies.exists(COOKIE_NAME)).to.be.false;
        });
      });
    ...
    describe('in a FastBoot-enabled app', function() {
      beforeEach(function() {
        this.fakeFastBoot = {
          isFastBoot: false,
        };
      ...
      itUsesBrowserAPIs.apply(this);
    });

    describe('in an app that is not FastBoot-enabled', function() {
      beforeEach(function() {
  ...
  describe('in the FastBoot server', function() {
    ...
    describe('reading a cookie', function() {
      ...
      it("doesn't decode the value when raw is true", function() {
        ...
        expect(this.cookies.read(COOKIE_NAME, { raw: true })).to.eq(encodeURIComponent(value));
      });

      it('returns undefined when the cookies does not exist', function() {
        expect(this.cookies.read('does-not-exist')).to.be.undefined;
      });
      ...
      it('respects the request headers after a cookie was written already', function() {
        ...
        expect(this.cookies.read()).to.deep.equal({ [COOKIE_NAME]: writtenValue, aRequestCookie: requestValue });
      });
